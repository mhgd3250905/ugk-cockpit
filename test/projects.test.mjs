...
test('historical projects migrate and removal survives a fresh process', (t) => {
  ...
  db.exec('ALTER TABLE projects DROP COLUMN removed_at; DELETE FROM schema_migrations WHERE version = 28; PRAGMA user_version = 27;');
  db.close();
  ...
  assert.deepEqual(JSON.parse(output.trim()), [0, 1]);
});
