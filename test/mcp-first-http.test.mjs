...
test('existing Agent initializes the registered project, continues, and hands off', async (t) => {
  ...
  // 错误目录的聊天不能接手派发（归属与写租约必须落在真实工作目录上）。
  // 未注册目录 → PROJECT_NOT_FOUND；已注册但别的项目 → DISPATCH_GRANT_BINDING_MISMATCH。
  const foreignDir = mkdtempSync(path.join(os.tmpdir(), 'ugk-cockpit-mcp-foreign-'));
  t.after(() => rmSync(foreignDir, { recursive: true, force: true }));
  const wrongDirAccept = await post(service, '/api/v1/mcp/work/accept', {
    dispatchCode: standbyCode,
    clientRequestId: 'accept-wrong-dir',
