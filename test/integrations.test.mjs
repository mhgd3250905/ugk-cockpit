...
test('command-backed receipt replay keeps one receipt despite a changed clock', (t) => {
  const db = fixture(t);
  const sub = createSubmission(db, {
    projectId: 'proj-integ',
    spaceId: 'space-feat-1',
  ...
  // mergeApprovedSubmission 以派生 commandId 调用且不传 receiptId：回执 id
  // 必须从稳定的 command id 派生，重放时即便时钟前移也保持 journal digest
  // 一致，否则崩溃恢复会永远卡在 COMMAND_CONFLICT。
  const request = {
