...
test('path guard accepts a normal path inside the granted root', (t) => {
  const root = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-path-'));
...
test('path guard normalizes dot segments, case variants, and long paths', (t) => {
  const root = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-long-path-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  let child = root;
...
test('path guard rejects a path outside the granted root', (t) => {
  const root = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-root-'));
  const outside = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-outside-'));
  t.after(() => {
...
test('path guard rejects a junction or symlink even when its target exists', (t) => {
  const root = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-link-root-'));
  const outside = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-link-target-'));
  t.after(() => {
    rmSync(root, { recursive: true, force: true });
    rmSync(outside, { recursive: true, force: true });
  });
...
test('path guard rejects a selected root that is itself a junction or symlink', (t) => {
  const container = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-link-container-'));
  const target = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-link-selected-target-'));
  t.after(() => {
...
test('path guard rejects a selected root below a junction or symlink ancestor', (t) => {
  const container = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-ancestor-link-container-'));
  const target = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-ancestor-link-target-'));
  t.after(() => {
    rmSync(container, { recursive: true, force: true });
    rmSync(target, { recursive: true, force: true });
  });
...
test('only one live instance owns the lock and a stale lock is recoverable', (t) => {
  const root = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-lock-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
...
test('an old owner cannot delete a replacement owner lock', (t) => {
  const root = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-lock-owner-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const lockPath = path.join(root, 'service.lock');
...
test('a partially written live lock cannot be stolen', (t) => {
  const root = mkdtempSync(path.join(fixtureTempRoot(), 'ugk-cockpit-lock-partial-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const lockPath = path.join(root, 'service.lock');
