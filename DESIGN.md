...
桌面端保留固定项目导航，正文限制宽度并居中于剩余空间。根据 2026-09-06 试用反馈，总览与开发空间采用紧凑卡片，时间线采用摘要优先的连续记录，避免条目随超宽屏无限拉伸。反馈与验收见 [工作台试用反馈](docs/WORKBENCH_FEEDBACK.md)。

```text
工作台 Shell
├── 固定左侧导航（236px）：项目总览、名称搜索、项目切换、主题、本地同步状态、使用指南
├── 窄顶部工具栏：返回总览、当前位置、手动刷新、添加项目
├── 全局通知：发生了什么、影响、下一步和可用重试
├── 项目总览：按行动状态分组的紧凑卡片，名称 / 状态与时间 / 两行进展 / 会话与真实操作
├── 项目详情页：独立 hash URL、紧凑标题、正常页面滚动
│   ├── 工作线（默认）：日期分段、多轨时间线、最新摘要展开、历史按需阅读、工作线聚焦与分页
...
- 项目卡片使用中性表面与细边框，状态以文字标签呈现；不铺满状态底色，也不加彩色左边条。
...
- 品牌 Logo 使用独立 SVG：C 形轮廓结合前进指针与记录节点，品牌区和浏览器图标共用 `web/public/assets/cockpit-mark.svg`，保持小尺寸辨识度。
...
| 区间 | 布局 |
| --- | --- |
| `> 1199px` | 固定 236px 左导航；正文上限 1504px；项目分组内自适应一至三列卡片；详情右栏约 228px |
| `1041–1199px` | 固定导航不变，缩减正文间距，详情右栏约 210px |
| `921–1040px` | 固定导航保留；上下文移到正文下方，以两列组织 |
| `<= 920px` | 导航改为顶部可展开区域，选择项目后收起；不覆盖正文 |
| `<= 700px` | 项目卡片单列；上下文单列；时间线保留图带与正常页面滚动 |

总览与详情共用 1504px 宽度上限，在导航右侧空间内居中。项目卡片基准最小宽度 352px，开发空间 260px；窄屏下允许缩小到容器宽度。成功反馈宽度上限 760px。摘要最多显示两行，完整内容在详情读取；长项目名、路径和正文须换行或局部省略，避免整页横向溢出。

工作线时间线在左侧保留稳定多轨图带，主项目使用暖橙，开发空间使用低饱和区别色。卡片共用同一左边界，按日期分段，显示归属、事件类型、时间与两行摘要；当前范围的最新事件适度展开，完整正文与技术证据按需读取。点击摘要或卡片空白处展开/收起记录；轨道节点和工作线按钮用于聚焦整条工作线，其他记录仍保持原位。「显示全部」恢复全线最新事件展开。按 `worktreeId`/开发空间身份归轨，不按相邻事件、Agent 或分支名推断关系；来源未知时使用独立中性轨。来源与接入曲线只依据真实记录；送审、审核通过、聊天接力和普通交接均不绘制合流或推断阶段完成。
