...
> 本文是 Phase 0 的时点快照，不是当前门禁的实时结果。当前开发版本一律以 `VERSION` 与[阶段记录](PHASE1_VERTICAL_SLICE.md)为准（本文不复述版本号：2026-09-26 复核时这里的复述已连续两次落后于实际）；Phase 0 门禁本身仍按 `npm run test:phase0` 运行，下面的数量与结论只在 2026-09-01 的那次运行成立。
...
该快照当时的全量结果：51 项通过，0 项失败，总耗时约 41 秒。（同一门禁在 2026-09-23 的 `main` 上为 97 项，数量差异来自后续阶段新增用例，不代表本轮回归。）
