...
2. 检查 Node.js（推荐仓库要求的 24.15.x）、Git 和当前宿主。Codex 需要支持 `codex plugin` 的 CLI；ZCode 使用其安装包自带的 CLI 与原生插件接口。缺少依赖时由 Agent 使用当前环境允许的官方安装方式准备；需要系统权限时说明具体缺失项。不得声称下载仓库即安装完成，也不能把 Codex 的配置格式套用到 ZCode。
