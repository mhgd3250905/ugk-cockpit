...
核对日期：2026-09-23，核对时的源码版本为 `0.1.0-alpha.51`；当前开发版本已是 `0.1.0-alpha.52`（见 `VERSION`）。

下表按宿主记录的是 `_meta` 字段形状，alpha.52 未改变各宿主能提供哪些字段，因此结论仍然有效。但 alpha.52 改变了平台对外的**呈现方式**：聊天归属凭据不再下发给非持有方，宿主/会话定位符默认遮蔽并返回 `identityWithheld: true`。逐宿主的真机验收**尚未在 alpha.52 上重跑**；需要定位符明文时按[本机服务恢复](LOCAL_SERVICE_RECOVERY.md)的流程重新核对。
