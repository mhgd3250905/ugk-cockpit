...
- 下一小步：MCP-first 工作闭环。网页只创建等待接手的任务；AI 成功调用本机 MCP 后才显示“正在工作”，不能因消息已复制就制造“已经开始”的错觉。
...
主按钮为 `交给 AI` 或 `继续工作`。用户选择 Agent 并填写一句任务目标，Cockpit 创建短期一次性接手码并生成可复制消息。系统先做只读预检：
...
消息复制后页面只显示“等待 AI 接手”。AI 必须先通过本机 MCP 接受任务，成功后才创建工作会话；工作中通过 MCP 报告进展。普通用户不需要理解 MCP、heartbeat、lease 或 snapshot。

### 5. 结束工作

AI 通过 MCP 选择：`已完成`、`卡住了`、`稍后继续`，提交摘要和下一步。系统重新只读采集代码状态并生成接手记录；网页并列显示“Agent 报告”和 Cockpit 验证结果，最终确认与接管仍由用户完成。
...
- 本机 stdio MCP 提供 `ugk_work_accept`、`ugk_work_progress`、`ugk_work_finish`，服务端从接手码或 session 解析项目和代码位置；
