...
本机 Antigravity IDE/CLI 程序均含该原生字段；CLI 真实工具请求已观测到该字段及稳定字符串值。验证只使用临时无业务副作用的 MCP 探针，保存字段名与身份摘要，不记录原始聊天 ID 或工具参数，不消费用户接力码。原 IDE 聊天须重新加载 bridge 后另行验收。常用宿主按官方接口分别核实，Claude Code 的出生进程环境变量、Gemini CLI 的 progressToken 不能替代当前聊天身份；Cursor 尚无已核实的原生字段，详见[支持清单](MCP_HOST_SUPPORT.md)。

验证（2026-09-20，Windows / Node.js 24.15.0，基于 `531377316a701b766fa979175d758c473a8eb66c` 的本轮工作树）：身份/stdio/持久绑定/转交回归 **57/57**；追加的原生 Antigravity 元数据跨宿主 HTTP 接力测试 **1/1**，验证旧 Codex 写入拒绝、服务与 bridge 重建后同聊天可继续及其他聊天隔离；`npm run test:phase0` **97/97**，版本一致性、隔离网页构建、58 处本地文档链接与差异检查通过。CLI 真实 MCP 探针 **4 次调用通过**：同聊天多次调用与 CLI 重建恢复的身份摘要一致，新聊天摘要不同，临时服务器配置已移除。未消费用户接力码，未更新正式 HTTP 服务或推送远端；原 Antigravity IDE 聊天须重连 MCP 后验收。
