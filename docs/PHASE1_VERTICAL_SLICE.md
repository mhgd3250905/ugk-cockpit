...
- 追加（PR 门禁自证）：本 PR 同码树两次 CI 运行一次失败一次通过，失败点是 `test/delivery-intake.test.mjs:61`「交付变更计数」多 1。本机用一份 `core.autocrlf=true` 的宿主 git 配置**确定性复现**该失败（`3 !== 2`），去掉后立即 12/12 通过：alpha.53 之后那轮 CI 修复给四个夹具文件加了「夹具 git 与服务端同样屏蔽 system/global 配置」的隔离，本文件是漏网的第五个——夹具按宿主配置垫出 CRLF，而产品视图看不到，幻影改动正好落在被计数的变更集里。现补上同一隔离（`GIT_CONFIG_NOSYSTEM` + `GIT_CONFIG_GLOBAL`），两种环境各 12/12。属测试基建，不改产品代码，因此上面 683 项全量与 97/97 的 Phase 0 证据继续适用，新头提交由 CI 重跑同一全量门禁复验。
