...
组合验收（Windows / Node.js 24.15.0，2026-09-11）：`npm test -- --test-concurrency=4` **563/563**，无失败或跳过，647.26 秒；`npm run test:phase0` **97/97**；`npm run build:web` 与差异检查通过。全量在候选 `8e729137762047a547a8f60be1d2bab42ce7e2f9` 加上述两个修复文件的工作树运行，随后与 `d63103f` 比较无差异，最终树为 `eb5c70ddbc6788c01919a8539476dc485e1ee8b2`。Phase 0 和构建先于补充修复运行；补充变化只涉及旧送审核心及回归，最终全量再次覆盖全部 Phase 0 用例。PR #10 的启动器复审 **7/7**、PR #11 的最终策略复审 **48/48** 属各自固定分支证据，不代替组合全量。
