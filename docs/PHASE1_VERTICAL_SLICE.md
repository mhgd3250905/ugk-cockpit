...
- 验证（2026-09-25，Windows / Node.js 24.15.0，本轮分支工作树）：全量 `npm test`（`--test-concurrency=1`）**683 项 / 676 通过 / 0 失败 / 7 平台跳过**，1366.3 秒，真实退出码 0；`npm run test:phase0` **97/97**（91.4 秒）；`npm run build:web -- --outDir <审核目录>` 通过（保留既有大包提示）；`git diff --check` 通过。基线为 pristine main `e3f144a` 的同机全量 **664 项 / 657 通过 / 0 失败 / 7 跳过**（1288.2 秒），本轮新增 19 项。抖动如实登记：`test/audit-2026-09-11.test.mjs:139` 的夹具自检（断言 git clean 过滤器确实执行过）在一次 681 项运行中失败过一次，单跑该文件 2/2、与相邻文件配对 19/19 均绿；该断言在任何产品代码之前且不在本轮改动路径上，同一断言在 alpha.45 段落记录过同样偶发且当时未确定原因，本轮不改他人夹具。
