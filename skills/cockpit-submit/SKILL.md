...
送审对象是用户确认的当前分支成果，不限于本聊天亲自完成的提交。若当前 HEAD 已超出用户指定的交付点，简要说明差异并确认本次版本；用户已明确选择当前分支全量成果时不重复追问。不能把其他会话的提交冒称为自己验收过，也不能仅因换过会话或 Git 作者相同/不同而拒绝接纳。实际写入权限由 MCP 核验。
...
3. 返回 `DELIVERY_SCOPE_REQUIRED` 时，核对完整 `changes` 与本次任务，明确列出将保存的文件；不相关、敏感或归属不明的未提交改动不能捎带。用新请求号和精确的 `files` 列表重新检查；`files: []` 只送审已提交代码，不保存未提交改动。主项目素材和未选中文件不属于这次内容检查范围。选择不明确时问用户。
...
`DELIVERY_CONTENT_TOO_LARGE` 应依据 MCP 的文件与限制详情说明选中文件超限，不以整个目录体积猜测原因，不建议清理 build 或删除无关素材来送审。若 `files: []` 仍报内容超限，或没有足够详情定位，报告平台预检异常，不扩大扫描、清理或反复盲试。
