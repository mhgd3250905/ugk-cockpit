...
import { createConversationControlState } from './conversation-control-state.mjs';
...
function ProjectDetailContent({ data, loadingMore, loadError, onLoadOlder, actionNotice, busy, onCreateSpace, onAssignSpace, onReuseSpace, onRemoveSpace, pendingWorkspaceActions = [], workspaceActionStorageError = null, onRecoverWorkspaceAction, onCopyReviewPrompt, onNoteStatusChange, onRecordsChanged, onLoadDiagnostics, diagnostics, diagnosticsLoading }) {
  ...
  const [conversationState] = useState(createConversationControlState);
...
function ConversationControlChain({ chain, path, onRefresh, sessionState }) {
  ...
  const { busy, error, issued, requestNotice } = useSyncExternalStore(
    sessionState.subscribe, sessionState.getSnapshot, sessionState.getSnapshot,
  );
  const setBusy = (value) => sessionState.set('busy', value);
  const setError = (value) => sessionState.set('error', value);
  const setIssued = (value) => sessionState.set('issued', value);
  const setRequestNotice = (value) => sessionState.set('requestNotice', value);
  const [copied, setCopied] = useState('');
  const pendingRequest = sessionState.pendingRequest;
  ...
  async function submit(retry = false) {
    if (sessionState.getSnapshot().busy) return;
    if (!retry) {
