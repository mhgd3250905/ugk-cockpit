...
已有本仓库的用户，也可以在程序目录执行对应命令（Windows 在 PowerShell、macOS 与 Linux 在终端中运行）。Linux 目前只覆盖服务与 MCP 安装路径：添加项目所需的原生文件夹选择器尚未实现（`FOLDER_PICKER_UNAVAILABLE`），而注册项目必须携带该选择器签发的一次性目录授权，**Linux 上因此暂无产品内的添加项目入口**（不是「改用手动指定目录」——这样的入口并不存在）。请在 Windows 或 macOS 上完成添加后，再到 Linux 上使用服务与 MCP；其余宿主能力与 macOS 一致：
