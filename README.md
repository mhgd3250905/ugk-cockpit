...
`0.1.0-alpha.33` — 项目运行详情的时间线按稳定工作副本绘制多条工作线：主项目与开发空间各自归轨，事件交错时仍保持来源可读；真实 `integrated` 回执才显示接入主项目关系，点击节点或卡片可突出整条工作线。分支变化不再被推断为 checkout 或切换。完整语义与验收边界见 [工作线时间线](docs/WORK_LINE_TIMELINE.md)。
