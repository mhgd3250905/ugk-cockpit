...
`0.1.0-alpha.41` — 合并 PR #8、#9 的审计修复：校验本机服务 Host，补齐受保护 Git 操作的配置检查，改善凭据文件写入中断恢复与单实例锁判断。浏览器请求增加截止时间并限制轮询堆积，目录选择保留 125 秒等待预算；修复安装器的非 Windows 路径处理。未登记工作链的旧运行记录可经用户确认释放残留写入锁，受管理工作会话仍走工作台转交。保留七技能使用指南，沿用 schema 27，工作台仍处于用户试用期。

本轮版本入口为 [alpha.41 预发布](https://github.com/mhgd3250905/ugk-cockpit/releases/tag/v0.1.0-alpha.41)。合并源码与发布整理不等于本机服务升级；本轮未重启正式服务或批量更新宿主插件。升级仍需核对已有项目及详情，验证范围见[阶段记录](docs/PHASE1_VERTICAL_SLICE.md)。
