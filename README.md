...
`0.1.0-alpha.35` — 轻量 Submit 工作说明（本地实现、验收与工作台切换完成）：向所属项目发布说明，可以引用 PR、本地提交或其他分支的审核结果，不再默认保存上传或创建代码审核对象。说明发布不冻结分支，不结束会话，已有 progress 与 relay 照常推进。`npm test` 289/289、Phase 0 90/90、隔离构建与浏览器验收通过；详见 [Submit 工作说明](docs/SUBMIT_NOTES.md)。2026-09-03 已切换本机服务与页面资源，并同步 Codex、共享 Agent 目录中的 `cockpit-submit` Skill；新启动的 MCP 已验证提供新版工具，已有客户端仍须重连 MCP 并重新加载 Skill。
