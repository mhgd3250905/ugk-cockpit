...
`0.1.0-alpha.25` — 前端改为深色优先的控制台视觉，并提供「亮色 / 暗色 / 跟随系统」三档手动切换。界面配色全部改为语义令牌，状态色不再铺满卡片，只用于左侧色条和状态徽标；项目卡片去掉固定高度，改为「状态 / 项目名 / 发生了什么 / 影响 / 下一步」的紧凑三行结构，一屏可以看到更多项目；运行时间线把原来的四种高饱和节点色收敛为形状加单一强调色，中文字重归一并去掉负字距，小字号不再发虚。
