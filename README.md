...
`0.1.0-alpha.29` — 统一分支送审：无论是否提前 init、是否由平台创建分支，用户完成任务后都可显式调用 `$cockpit-submit`。平台先核对已登记项目、当前代码权限、本次文件范围及最新远端 main，再保存必要提交、普通推送并生成主项目审核待办。重复送审复用记录，新版本使旧审核失效；冲突不会被标成可合并。
