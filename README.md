...
> 上面这行的「仍在 alpha.48 / schema 29」只是 2026-09-22 的现场快照，已被取代：2026-09-23 用户授权部署后，本机服务运行 alpha.51，正式库已完成 schema 29 → 30 迁移。当前运行版本与核对记录以[本机服务恢复](docs/LOCAL_SERVICE_RECOVERY.md)的「alpha.51 部署与 schema 30 迁移」小节为唯一事实源；alpha.52 尚未部署，且不含新的 schema 迁移。
...
保留 `alpha.39` 的工作节点持久记录平台与宿主会话 ID，接手成功本身就是新节点。异常转交由用户在工作台选择准确工作会话并授权：旧聊天立即冻结，新聊天凭一次性指令接手；支持定向授权、过期重签和明确取消恢复。聊天内两步 takeover 已关闭，正常 Relay 保留。ZCode 原生请求身份已接入；走错聊天时返回当前持有人和最新节点，未知历史不猜身份。本机服务当时已使用 schema 29（该状态已被 2026-09-23 的 alpha.51 / schema 30 部署取代，见上方 alpha.49 小节后的说明）；此前用户反馈原 ZCode 聊天接手后可写属于当时的现场验收，服务更新不代表宿主插件同步更新。协议与运维见[会话身份与中断恢复](docs/CONVERSATION_DURABILITY.md)和[本机服务恢复](docs/LOCAL_SERVICE_RECOVERY.md)。
