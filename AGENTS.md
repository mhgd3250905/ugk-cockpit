...
- Node.js 支持范围为 `>=24.15.0 <25`。Phase 0 后端仅使用内置模块；Phase 1 网页依赖以 `package.json` 为准。
- 数据库迁移必须可重复执行；状态转换使用事务、幂等键和 revision CAS。
- 工作会话归属、业务状态和操作恢复依据不得只存于进程内存。聊天身份与 MCP 连接生命周期分离；支持的宿主通过请求元数据恢复平台持久绑定，不以目录或 PID 猜测原聊天。内存缓存必须可从事实源重建。
- 修改持久性或恢复协议时，覆盖有历史数据的迁移、真实进程重建及接力失效；文件写入崩溃场景不能仅以会执行 finally 的异常测试替代进程终止。当前契约见 `docs/CONVERSATION_DURABILITY.md`。
